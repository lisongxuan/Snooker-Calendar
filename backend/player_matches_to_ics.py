...
def generate_player_calendar(player_id, year, headers=None, max_retries=3, timeout=15):
    ...
    for attempt in range(max_retries):
        ...
        if matches is not None:
            break  # Got a usable result (non-empty match list)

        # The snooker library returns None both when the API is rate-limited
        # (403.502 HTML page -> JSON decoding fails) and when the player has no
        # matches for the season. Wait out the rate-limit window before retrying
        # (api.snooker.org allows roughly 2 requests/minute per IP).
        if attempt < max_retries - 1:
            wait_time = 65 if attempt == 0 else 2 ** (attempt + 1)
            print(f"  Attempt {attempt + 1}/{max_retries} returned no data (likely API rate-limit); retrying in {wait_time}s...")
            time.sleep(wait_time)
            continue

        print(f"  No data after {max_retries} attempts (rate-limited or no matches this season).")
