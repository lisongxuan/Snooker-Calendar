...
def generate_all_players_calendars(year=None, cancel_check=None):
    """
    为所有活跃玩家生成ICS日历文件。
    ...
    for i, player in enumerate(players, 1):
        ...
        # 取消检查点: 维护窗口(events/rankings)到来时, 在当前球员后停止(有界, 最多再等约1名球员)
        if cancel_check is not None and cancel_check():
            print(f"[{i}/{total_count}] ICS generation cancelled by maintenance job - stopping now")
            break
